...
| **NAT gateway is the chunky cost** | The networking stack creates a NAT (~$0.045/hr in us-east-1, or 2× when `NatGatewayCount=2`) regardless of whether the service is doing anything. Tear down when not in use; see `teardown.md`. |
